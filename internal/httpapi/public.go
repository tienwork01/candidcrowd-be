...
func (h *PublicHandler) Event(c *gin.Context) {
	...
	if err != nil {
		RespondError(c, NewError(http.StatusNotFound, "not_found", "event not found"))
		return
	}
...
func (h *PublicHandler) CreateUpload(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		RespondError(c, NewError(http.StatusBadRequest, "invalid_request", err.Error()))
		return
	}
	e, err := h.events.GetPublic(c.Request.Context(), c.Param("slug"))
	if err != nil {
	...
	if err != nil {
		RespondError(c, NewError(http.StatusUnauthorized, "invalid_guest_session", "guest session is invalid or expired"))
		return
	}
	...
	if err != nil {
		RespondError(c, NewError(http.StatusUnprocessableEntity, "upload_not_allowed", err.Error()))
		return
	}
...
func (h *PublicHandler) Complete(c *gin.Context) {
	...
	if err != nil {
		RespondError(c, NewError(http.StatusBadRequest, "invalid_id", "upload id must be a UUID"))
		return
	}
	e, err := h.events.GetPublic(c.Request.Context(), c.Param("slug"))
	if err != nil {
	...
	if err = h.media.Complete(c.Request.Context(), e, session, id); err != nil {
		if err == gorm.ErrRecordNotFound {
			RespondError(c, NewError(http.StatusNotFound, "not_found", "upload not found"))
		} else {
			RespondError(c, NewError(http.StatusUnprocessableEntity, "upload_verification_failed", err.Error()))
		}
		return
	}
